...
const Leaderboard: React.FC = () => {
  ...
  const { users, loading, saving, refreshEntries, saveEntry, getEntryForDate, entries } = useMeditation(currentUser); 
  ...
  // Show spinner for initial load only
  if (loading && !users.length) { // Check if users array is empty to determine initial load
    return (
  ...
  return (
    <>
      <ProgressBar visible={loading || saving} position="top" />
        ...
        {currentUser && (
          <div className="mb-8 px-4 py-3 bg-white shadow rounded-lg">
            ...
            <LogMeditationForm 
              ...
              onSave={saveEntry} 
              isSaving={saving} // Use isSaving from useMeditation hook
      ...
      <ProgressBar visible={loading || saving} position="bottom" />
    </>
  );
};
